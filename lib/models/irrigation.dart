...
class ZoneIrrigation {
  ...
  // Copie avec un seuil dynamique (venant du backend)
  ZoneIrrigation withSeuil(int seuil) => ZoneIrrigation(
    nom:       nom,
    humidite:  humidite,
    surface:   surface,
    statut:    humidite < seuil ? 'alerte' : statut,
    capteurId: capteurId,
    seuilMin:  seuil,
  );
}
...
// Utilisées si le backend est inaccessible ou ne retourne rien
// ─────────────────────────────────────────────────────────────────────────────

const double seuilHumiditeStatique = 60.0;
const bool   irrigationAutoStatique = true;

final List<ZoneIrrigation> zonesIrrigationStatiques = [
  const ZoneIrrigation(nom: 'Zone A', humidite: 74, surface: 0.8, statut: 'ok',     capteurId: 'C1', seuilMin: 60),
  const ZoneIrrigation(nom: 'Zone B', humidite: 48, surface: 1.1, statut: 'alerte', capteurId: 'C3', seuilMin: 60),
  const ZoneIrrigation(nom: 'Zone C', humidite: 81, surface: 1.2, statut: 'ok',     capteurId: 'C5', seuilMin: 60),
  const ZoneIrrigation(nom: 'Zone D', humidite: 65, surface: 0.9, statut: 'ok',     capteurId: 'C7', seuilMin: 60),
...
// Variable globale mutable — mise à jour par IrrigationScreen après chargement
// (permet aux autres écrans d'y accéder sans refetch)
List<ZoneIrrigation> zonesIrrigation = List.from(zonesIrrigationStatiques);
