...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  double? _humidite;       // null = pas de donnée
  double? _temperature;    // null = pas de donnée
  int     _capteursActifs     = 0;
  int     _capteursTotalCount = 0;
  int     _alertesActives     = 0;
  int     _seuil              = 0;
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      // 2. Utilisateur depuis AuthService (pas de requête supplémentaire)
      final user = auth.user;
      if (user != null) {
        _prenomUtilisateur = user.prenom.isNotEmpty ? user.prenom : user.nom;
      ...
      final total           = capteurs.length;
      final actifs          = capteurs.where((c) => c.etat == 'actif').length;
      final alertesBatterie = capteurs
          .where((c) => c.etat != 'inactif' && (c.batterie as int? ?? 100) <= 20)
          .length;
      final inactifs        = capteurs.where((c) => c.etat == 'inactif').length;
      int alertesCount      = alertesBatterie + inactifs;

      // 4. Seuil
      final seuilVal = seuil != null ? seuil.valeurMin.toInt() : 0;

      // 5. Mesures réelles — humidité + température en parallèle sur tous les capteurs
      double? humMoy, tempMoy;
      List<double> historiqueHum = [];

      if (capteurs.isNotEmpty) {
        final capteurIds = capteurs.map<String>((c) => c.id as String).toList();

        // Dernière mesure de chaque capteur, toutes en parallèle
        ...
        // Calcul des moyennes (on ignore les capteurs sans mesure)
        ...
        if (humsValides.isNotEmpty) {
          humMoy = humsValides.reduce((a, b) => a + b) / humsValides.length;
        }
        if (tempsValides.isNotEmpty) {
          tempMoy = tempsValides.reduce((a, b) => a + b) / tempsValides.length;
        }

        // Historique du premier capteur actif pour le graphique
        final premierActif = capteurs.firstWhere(
          (c) => c.etat == 'actif',
          orElse: () => capteurs.first,
        );
        final historique = await mesureService.getHistoriqueHumidite(
          premierActif.id as String,
        );
        if (historique.isNotEmpty) {
          final pts = historique.reversed.take(7).toList().reversed.toList();
          historiqueHum = pts.map((m) => m.valeur / 100.0).toList();
        }
      }
      ...
      if (humMoy != null && seuilVal > 0 && humMoy < seuilVal) {
        alertesCount++;
        alertes.add({
          'titre':   'Humidité critique — ${humMoy.toStringAsFixed(1)}% '
                     '(seuil : $seuilVal%)',
          'temps':   'Maintenant',
          'couleur': 'rouge',
        });
      ...
      for (final c in capteurs.where(
          (c) => c.etat != 'inactif' && (c.batterie as int? ?? 100) <= 20)) {
        alertes.add({
          'titre':   'Capteur ${c.nom} — batterie faible (${c.batterie}%)',
          'temps':   'Récent',
          'couleur': 'ambre',
  ...
  String get _titreAppBar => _prenomUtilisateur.isNotEmpty
      ? '$_salutation, $_prenomUtilisateur 👋'
      : '$_salutation 👋';

  String get _sousTitre => _nomUtilisateur.isNotEmpty
      ? 'Tableau de bord · $_nomUtilisateur'
      : 'Tableau de bord';
  ...
  String _valHumidite()    => _humidite    != null ? _humidite!.toStringAsFixed(1)    : '--';
  String _valTemperature() => _temperature != null ? _temperature!.toStringAsFixed(1) : '--';

  String _badgeHumidite() {
    if (_humidite == null || _seuil == 0) return '--';
    return _humidite! >= _seuil ? '↑ Normal' : '↓ Critique';
  }

  String _badgeTemperature() {
    if (_temperature == null) return '--';
    if (_temperature! > 35)   return '⚠ Élevée';
    if (_temperature! < 15)   return '↓ Basse';
    return '✓ Normale';
  }
  ...
  @override
  Widget build(BuildContext context) {
    final humOk  = _humidite    == null || _seuil == 0 || _humidite!  >= _seuil;
    final tempOk = _temperature == null || _temperature! <= 35;
                ...
                children: [
                  ...
                  GridView.count(
                    ...
                    children: [
                      ...
                      // Humidité moyenne réelle
                      MetricCard(
                        icon: Icons.water_drop_outlined,
                        iconBg:    humOk ? AppColors.green100 : AppColors.red100,
                        iconColor: humOk ? AppColors.green700 : AppColors.red800,
                        value: _valHumidite(),
                        unit:  _humidite != null ? '%' : '',
                        label: 'Humidité moy.',
                        badge: _badgeHumidite(),
                        badgeBg:   humOk ? AppColors.green100 : AppColors.red100,
                        badgeText: humOk ? AppColors.green700 : AppColors.red800,
                      ),

                      // Température moyenne réelle
                      MetricCard(
                        icon: Icons.wb_sunny_outlined,
                        iconBg:    tempOk ? AppColors.amber100 : AppColors.red100,
                        iconColor: tempOk ? AppColors.amber800 : AppColors.red800,
                        value: _valTemperature(),
                        unit:  _temperature != null ? '°C' : '',
                        label: 'Température moy.',
                        badge: _badgeTemperature(),
                        badgeBg:   tempOk ? AppColors.amber100 : AppColors.red100,
                        badgeText: tempOk ? AppColors.amber800 : AppColors.red800,
                      ),

                      // Capteurs actifs
                      MetricCard(
                        icon: Icons.sensors,
                        iconBg:    badgeCapteursBg,
                        iconColor: badgeCapteursText,
                        value: _capteursTotalCount > 0
                            ? '$_capteursActifs'
                            : '--',
                        unit: _capteursTotalCount > 0
...
class HumidityChartPainter extends CustomPainter {
  final int seuil;
  final List<double> points; // valeurs 0.0–1.0

  const HumidityChartPainter({this.seuil = 60, this.points = const []});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Données réelles ou ligne neutre si vide
    final pts = points.isNotEmpty
        ? points
        : List.generate(7, (_) => 0.5);

    final offsets = pts.asMap().entries.map((e) => Offset(
          leftPadding +
              e.key * (size.width - leftPadding) / (pts.length - 1),
  ...
  @override
  bool shouldRepaint(HumidityChartPainter old) =>
      old.seuil != seuil || old.points != points;
}*/